...
add_filter( 'wp_title', 'wp_title_custom', 10, 3 );
...
function get_option_mapa() {
    ...
    if ($option and $api_maps) {
        ...
        <script src='http://maps.googleapis.com/maps/api/js?key=".$api_maps."&callback=initMapa'></script>";
    }
}
...
function social_share($share) {
    ...
    foreach ($share as $rede) {
        if ($rede == 'facebook') {
            $html .= '<li class="facebook"><a href="http://www.facebook.com/sharer.php?u=' . $encoded_url . '" target="_blank" title="Facebook"><i class="icon-facebook"></i></a></li>';
        } elseif ($rede == 'twitter') {
            ...
            $html .= '<li class="pinterest"><a href="http://www.pinterest.com/pin/create/button/?url=' . $encoded_url . '&media=' . $media . '&description=' . urlencode( $title . ' - ' . $url ) . '" target="_blank" title="Pinterest"><i class="icon-pinterest"></i></a></li>';
        } elseif ($rede == 'email') {
