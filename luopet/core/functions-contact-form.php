...
class Contact_Form extends Front_End_Form {
	...
	protected function build_mail_message( $submitted_data, $attachments ) {
		...
		$message = apply_filters( 'contact_form_message_header_' . $this->id, '' );
		...
		foreach ( $data as $label => $value ) {
			if ( 'text/html' == $this->content_type ) {
				$message .= '<p><strong>'.$label.'</strong>: <br>'.$value.'</p>';
			} else {
		...
		$message .= apply_filters( 'contact_form_message_footer_' . $this->id, '' );
